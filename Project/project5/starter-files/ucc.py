...
def uc_compile(filename, analyze_only, write_types, write_graph,
    ...
    """Run the uC compiler on the given source file. If analyze_only
    is false and no errors are detected, writes generated code to an
    output file (backend project only). If write_types is true, writes
    a representation of the AST produced from parsing the source file,
    with annotated type information, to an output file. If write_graph
    is true, writes a representation of the AST that can be processed
    by GraphViz's dot tool to an output file."""
...
def uc_frontend(filename, write_types, write_graph, frontend_phase):
    """Run the uC compiler frontend on the given source file. If
    ...
    file. Returns the resulting AST and global environment."""
    import ucparser
    import ucfrontend
...
def uc_backend(filename, tree, global_env, backend_phase):
    """Run the uC compiler backend on the given AST and global
    environment. Writes generated code to an output file (backed
    project only)."""
    import ucbackend
    outname = (filename[:-3] if filename.endswith('.uc')
               else filename) + '.cpp'
    ...
    with open(outname, 'w') as out:
        ...
        for phase in (phases[:backend_phase] if backend_phase else phases):
...
def main():
    ...
    import argparse
    aparser = argparse.ArgumentParser(description='Compile a uC source file.')
    ...
    aparser.add_argument('-T', '--write-types', action='store_true',
                         help='write types for each expression and declaration')
    aparser.add_argument('-G', '--write-graph', action='store_true',
                         help='generate a graph of the AST in dot format')
    aparser.add_argument('-NE', '--no-errors', action='store_true',
                         help='disable error checking')
    aparser.add_argument('--frontend-phase', nargs='?', type=int, default=0,
                         help='stop semantic analysis after the given ' +
                         'frontend phase')
    aparser.add_argument('--backend-phase', nargs='?', type=int, default=0,
                         help='restrict code generation to the given ' +
                         'backend phase')
    args = aparser.parse_args()
    if args.code_gen and not args.frontend_phase:
        # restrict frontend to first two phases, with no error checking
    ...
    uc_compile(args.filename, args.analyze_only, args.write_types,
               args.write_graph, args.frontend_phase, args.backend_phase)
