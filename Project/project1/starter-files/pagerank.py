...
def pagerank(graph, num_iterations=40, d=.85):
...
def print_ranks(ranks, max_nodes=20):
    """Prints out the node IDs and rank values in the given
    dictionary, for the max_nodes highest ranked nodes, as well as the
    sum of all rank values. If max_nodes is not in [0, len(ranks)],
    prints out all nodes' rank values."""
    if max_nodes not in range(len(ranks)):
        max_nodes = len(ranks)
    # sort ids highest to lowest primarily by rank value, secondarily
    # by id itself
    sorted_ids = sorted(ranks.keys(), key=lambda n: (ranks[n], n),
                        reverse=True)
    for node_id in sorted_ids[:max_nodes]:
        print('{}: {:.5f}'.format(node_id, ranks[node_id]))
    if max_nodes < len(ranks):
        print('...')
    # compute sum using sorted ids to bypass randomness in dict
    # implementation
    print('Sum: {:.5f}'.format(sum(ranks[n] for n in sorted_ids)))

def pagerank_from_csv(node_file, edge_file, num_iterations):
    """Reads a digraph from CSV node/edge files in the format
    enumerated in the spec. Runs the PageRank algorithm for
    num_iterations on the resulting graph. Prints out the node ID and
    its PageRank for the 20 highest-ranked nodes, or all nodes if the
    graph has fewer than 20 nodes, to standard out. Also prints out
    the sum of all the PageRank values, which should approximate to
    1."""
    import graph
